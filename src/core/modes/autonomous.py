...
from src.core.config import MOTOR_CFG, AUTONOMOUS_CFG
...
async def navigate_step(controller, obstacle):
    if obstacle.is_blocked():
        ...
        # Begin reversing immediately — scan while moving so the maneuver flows smoothly
        controller.backward(REVERSE_SPEED)

        controller.move_camera_to("x", 45)
        await asyncio.sleep(0.8)                   # camera moves + robot reverses
        ...
        # Apply steer mid-reverse to begin the arc, then finish the K-turn
